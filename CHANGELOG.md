...
## [1.0.10] - 2026-04-10
* **[Optimized]:** Implemented strict Promise coalescing in `WebFetcher` URL validation to prevent SSRF DNS rebinding vulnerabilities. The cached Promise is now immediately removed from `hostValidationCache` upon resolution via `.finally()`.
* **[Optimized]:** Added URL pre-deduplication using `new Set()` in `WebFetcher.fetchBatch` to optimize batch fetching overhead.
