...
describe("ConfigManager", () => {
	...
	it("should propagate directory creation error", async () => {
		const error = new Error(
			"EACCES: permission denied, mkdir '/root/.autoresearch'",
		);
		vi.spyOn(fs, "mkdir").mockRejectedValueOnce(error);

		await expect(
			configManager.setConfig({ OPENAI_API_KEY: "test" }),
		).rejects.toThrow("EACCES: permission denied, mkdir '/root/.autoresearch'");
	});

	it("should cache configuration in memory to prevent redundant file reads", async () => {
		vi.spyOn(fs, "readFile").mockResolvedValue(
			'{"OPENAI_API_KEY":"cached-key"}',
		);
	...
	it("should create directory and save config successfully", async () => {
		...
		expect(fs.mkdir).toHaveBeenCalledWith(
			path.join(os.homedir(), ".autoresearch"),
			{ recursive: true },
		);
